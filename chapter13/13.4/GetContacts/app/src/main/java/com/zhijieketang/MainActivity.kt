...
class MainActivity : AppCompatActivity(), LoaderManager.LoaderCallbacks<Cursor> {
        ...
        mListView?.onItemLongClickListener =
            OnItemLongClickListener { parent, view, position, id ->
                ...
                if (cursor?.moveToFirst()!!) {
                    ...
                    // 查看Email
                    val emails = findEmail(contactId)
                    if (emails.isEmpty()) {
                        makeText(this, "$contactName 没有Email", LENGTH_LONG).show()
                    } else {
                        makeText(this, "$contactName ${emails}", LENGTH_LONG).show()
                    }
                }
                // Lambda表达式返回值
                true
     ...
     */
    private fun findEmail(contactId: String): List<String> {
